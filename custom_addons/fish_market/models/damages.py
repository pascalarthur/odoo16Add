...
class DamageOperation(models.Model):
    ...
    name = fields.Char(string="Reference", required=True, copy=False, readonly=True, index='trigram',
                       default=lambda self: default_name(self, prefix='DAM/'))

    state = fields.Selection(selection=[('draft', 'Draft'), ('confirmed', 'Confirmed')], string="Status", readonly=True,
