...
class ProductOfferWizard(models.TransientModel):
    ...
    available_product_pricelist_item_ids = fields.Many2many('product.pricelist.item', string='Available Offers',
                                                            readonly=False)
    ...
    def confirm_selection(self, form_name='product_offer'):
        ...
        for partner_id in self.partner_ids:
            token_record = self.env['access.token'].create([{
                ...
                'expiry_date':
                fields.Datetime.add(datetime.now(), days=30),  # 30 day validity
                'product_pricelist_item_ids':
                self.available_product_pricelist_item_ids.ids,
            }])
