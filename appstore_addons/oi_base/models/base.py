...
class Base(models.AbstractModel):
    ...
    def _suggest_external_id(self):
        ...
        return normilize(
            self._name, 'code' in self and isinstance(self['code'], str) and len(self['code']) < 20 and self['code']
            or '', self.display_name, record_id=self.id)
    ...
    def _create_external_id(self):
        ...
        if not ref:
            ...
            if self._context.get('is_approval_setting'):
                vals = {'module': '_workflow', 'name': self._suggest_external_id(), 'noupdate': True}
            else:
    ...
    def get_record_info(self):
        ...
        model_data = self.env['ir.model.data'].sudo().search([('model', '=', self._name), ('res_id', '=', self.id)],
                                                             order='id', limit=1)
        if model_data:
        ...
        if 'audit.log' in self.env:
            log_ids = self.env['audit.log'].sudo().search([('record_id', '=', self.id), ('model_id', '=', model_id),
                                                           ('type', '=', '3')]).ids
            data['log_count'] = self.env['audit.log'].sudo().search_count([('record_id', '=', self.id),
                                                                           ('model_id', '=', model_id)])

            message_ids = self.env['mail.message'].search([('model', '=', self._name), ('res_id', '=', self.id)])
            data['tracking_value_count'] = self.env['mail.tracking.value'].search_count([('mail_message_id', 'in',
                                                                                          message_ids.ids)])
        ...
        if log_ids:
            ...
            for detail in self.env['audit.log.detail'].sudo().search([('log_id', 'in', log_ids)], order='id desc',
                                                                     limit=100):
                data['lines'].append({
        ...
        if message_ids:
            tracking_value_ids = self.env['mail.tracking.value'].search([('mail_message_id', 'in', message_ids.ids)],
