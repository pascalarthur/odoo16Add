...
class NumberToArabic():
    ...
    arabicFeminineOnes = [
        "", "إحدى", "اثنتان", "ثلاث", "أربع", "خمس", "ست", "سبع", "ثمان", "تسع", "عشر", "إحدى عشرة", "اثنتا عشرة",
        "ثلاث عشرة", "أربع عشرة", "خمس عشرة", "ست عشرة", "سبع عشرة", "ثماني عشرة", "تسع عشرة"
    ]
    ...
    def numberToArabic(self, number, currency, englishPrefixText, englishSuffixText, arabicPrefixText,
                       arabicSuffixText):
    ...
    def getDecimalValue(self, decimalPart):
        ...
        if self.currencyInfo.getPartPrecision() != len(decimalPart):
            ...
            dec = len(decimalPart) <= self.currencyInfo.getPartPrecision() and len(
                decimalPart) or self.currencyInfo.getPartPrecision()
        ...
        if (tens > 0):
            if (tens < 20):  # if we are processing under 20 numbers
                if (tens == 2 and hundreds == 0
                        and groupLevel > 0):  #// This is special case for number 2 when it comes alone in the group
                    if (_intergerValue == 2000 or _intergerValue == 2000000 or _intergerValue == 2000000000
                            or _intergerValue == 2000000000000 or _intergerValue == 2000000000000000
                    ...
                    if (tens == 1 and groupLevel > 0 and hundreds == 0):
                        ...
                        if ((tens == 1 or tens == 2) and (groupLevel == 0 or groupLevel == -1) and hundreds == 0
                                and remainingNumber == 0):
                            retVal += ""  #// Special case for 1 and 2 numbers like: ليرة سورية و ليرتان سوريتان
                        else:
                            retVal += self.getDigitFeminineStatus(tens,
                                                                  groupLevel)  #// Get Feminine status for this digit
    ...
    def convertToArabic(self, value, currencyCode='SAR'):
        ...
        self.numberToArabic(self.number, self.currencyInfo, self.englishPrefixText, self.englishSuffixText,
                            self.arabicPrefixText, self.arabicSuffixText)
                    ...
                    if (numberToProcess != 2):
                        ...
                        if (numberToProcess % 100 != 1):
                            ...
                            if (numberToProcess >= 3
                                    and numberToProcess <= 10):  #// for numbers between 3 and 9 we use plural name
