...
class ReportAgedPartnerBalance(models.AbstractModel):
    ...
    def _get_partner_move_lines(self, account_type, date_from, target_move, period_length):
        ...
        cr.execute('SELECT debit_move_id, credit_move_id FROM account_partial_reconcile'
                   ' where max_date > %s', (date_from, ))
        ...
        for line in self.env['account.move.line'].browse(aml_ids):
            ...
            line_amount = ResCurrency._get_conversion_rate(line.company_id.currency_id, user_currency, line.balance)
            if user_currency.is_zero(line_amount):
                continue
            for partial_line in line.matched_debit_ids:
                if partial_line.max_date <= date_from:
                    line_amount += ResCurrency._get_conversion_rate(partial_line.company_id.currency_id, user_currency,
                                                                    partial_line.amount)
        ...
        for i in range(5):
            ...
            for line in self.env['account.move.line'].browse(aml_ids):
                ...
                for partial_line in line.matched_debit_ids:
                    if partial_line.max_date <= date_from:
                        line_amount += ResCurrency._get_conversion_rate(partial_line.company_id.currency_id,
                                                                        user_currency, partial_line.amount)
                for partial_line in line.matched_credit_ids:
                    if partial_line.max_date <= date_from:
                        line_amount -= ResCurrency._get_conversion_rate(partial_line.company_id.currency_id,
                                                                        user_currency, partial_line.amount)
