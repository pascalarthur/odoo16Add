...
class CurrencyInherit(models.Model):
    ...
    def _compute_current_rate(self):
        ...
        for currency in self:
            currency.rate = currency_rates.get(to_currency.id) * (currency_rates.get(currency.id) or 1.0)
    ...
    def _process_payment_lines(self, pos_order, order, pos_session, draft):
        ...
        if not draft and not float_is_zero(pos_order['amount_return'], prec_acc):
            ...
            if pos_order.get('currency_amount') and pos_order.get('currency_symbol'):
                ...
                if oc != sc:
                    return_amount = sc._convert(pos_order.get('amount_return', 0), oc, order.company_id,
                                                order.date_order)
