        ...
        if self.ks_name in ('Executive Summary', 'Balance Sheet', 'Profit and Loss', 'Cash Flow Statement'):
            ...
            if self.display_name != "Executive Summary":
                lines, ks_initial_balance, ks_current_balance, ks_ending_balance = self.with_context(
                    no_format=True, print_mode=True,
                    prefetch_fields=False).ks_fetch_report_account_lines(ks_df_informations)
            else:
            ...
            ks_new_start_date = (datetime.datetime.strptime(str(f_s_date), '%Y-%m-%d').date()).strftime(lang_id)
            ks_new_end_date = (datetime.datetime.strptime(str(f_e_date), '%Y-%m-%d').date()).strftime(lang_id)
            ...
            if ks_df_informations['ks_diff_filter']['ks_diff_filter_enablity']:
                if not ks_df_informations['ks_differ'].get('ks_intervals'):
                    ...
                    ks_new_start_comp_date = (datetime.datetime.strptime(str(f_start_date),
                                                                         '%Y-%m-%d').date()).strftime(lang_id)
                    ks_new_end_comp_date = (datetime.datetime.strptime(str(f_end_date),
                                                                       '%Y-%m-%d').date()).strftime(lang_id)
                else:
                    ...
                    ks_new_start_comp_date = (datetime.datetime.strptime(str(for_s_start_comp_date),
                                                                         '%Y-%m-%d').date()).strftime(lang_id)
                    ks_new_end_comp_date = (datetime.datetime.strptime(str(for_e_end_comp_date),
                                                                       '%Y-%m-%d').date()).strftime(lang_id)
            if self.display_name != 'Executive Summary':
                if not ks_df_informations['ks_diff_filter']['ks_diff_filter_enablity']:
                    if ks_df_informations['date']['ks_process'] == 'range':
                        ...
                        if ks_df_informations['date'].get('ks_end_date'):
                            sheet.write_string(row_pos_2 + 1, row_pos_2, ks_new_end_date, content_header_date)
                    # Accounts
                    row_pos_2 += 1
                ...
                row_pos_2 += 0
                sheet.write_string(2 - 2, 2, _('Journals'), format_header)
                j_list = ', '.join(
                    journal.get('code') or '' for journal in ks_df_informations['journals'] if journal.get('selected'))
                sheet.write_string(1, 2, j_list, content_header)
                ...
                sheet.write_string(1, 3, j_list, content_header)
                ...
                j_list = ', '.join(
                    journal.get('name') or '' for journal in ks_df_informations['account'] if journal.get('selected'))
                sheet.write_string(2 - 1, 3, j_list, content_header)
                if ks_df_informations['ks_diff_filter']['ks_debit_credit_visibility']:
                ...
                if not ks_df_informations['ks_diff_filter']['ks_debit_credit_visibility']:
                    ...
                    sheet.write_string(row_pos, 1, ks_df_informations['date']['ks_string'], format_header)
                    ks_col = 2
                    # for x in range(3):
                    for i in ks_df_informations['ks_differ']['ks_intervals']:
                        sheet.write_string(row_pos, ks_col, i['ks_string'], format_header)
                        sheet.set_column(ks_col, ks_col, 20)
                        ks_col += 1

                    ks_col_line = 0
                    for line in lines:
                        sheet.write(row_pos + 1, 0, line['ks_name'], line_header_string)
                        if line.get('balance'):
                            for ks in line.get('balance'):
                                sheet.write(row_pos + 1, ks_col_line + 1, line.get('balance')[ks], line_header)
                                ks_col_line += 1
                        ks_col_line = 0
                        row_pos += 1
