...
class AccountPayment(models.Model):
    ...
    search_statement_matched_payment_id = fields.Many2one("account.bank.statement.line", store=False,
                                                          search="_search_statement_matched_payment_id")

    def _search_statement_matched_payment_id(self, operator, value):
        line_id = self.env["account.bank.statement.line"].browse(value)
        domain = [('journal_id', '=', line_id.journal_id.id), ('is_matched', '=', False), ('state', '=', 'posted'),
                  line_id.amount > 0 and ('payment_type', '=', 'inbound') or ('payment_type', '=', 'outbound')]
