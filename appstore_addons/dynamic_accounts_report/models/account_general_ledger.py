...
class AccountGeneralLedger(models.TransientModel):
    ...
    def view_report(self, option, tag):
        ...
        account_dict = {}
        account_totals = {}
        move_line_ids = self.env['account.move.line'].search([('parent_state', '=', 'posted')])
        ...
        for account in account_ids:
            ...
            for move_line in move_line_id:
                move_line_data = move_line.read([
                    'date', 'name', 'move_name', 'debit', 'credit', 'partner_id', 'account_id', 'journal_id', 'move_id',
                    'analytic_line_ids'
                ])
    ...
    def get_filter_values(self, journal_id, date_range, options, analytic, method):
        ...
        if analytic:
            analytic_line = self.env['account.analytic.line'].search([('account_id', 'in', analytic)]).mapped('id')
        ...
        if date_range:
            if date_range == 'month':
                ...
                domain += [('date', '>=', last_month_start), ('date', '<=', last_month_end)]
                ...
                domain += [('date', '>=', last_year_start), ('date', '<=', last_year_end)]
        ...
        if data:
            if report_name == 'General Ledger':
                ...
                for account in data['account']:
                    ...
                    sheet.merge_range(row, col + 7, row, col + 8, data['total'][account]['total_debit'], txt_name)
                    sheet.merge_range(row, col + 9, row, col + 10, data['total'][account]['total_credit'], txt_name)
                    ...
                    for rec in data['data'][account]:
                        ...
                        sheet.merge_range(row, col + 2, row, col + 4, rec[0]['name'], txt_name)
