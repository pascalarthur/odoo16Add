...
class TaxReport(models.TransientModel):
    ...
    def view_report(self):
        ...
        for tax in tax_ids:
            tax_id = self.env['account.move.line'].search([('tax_ids', '=', tax.id), ('parent_state', '=', 'posted'),
            ...
            if tax.type_tax_use == 'sale':
                sale.append({
                    ...
                    'tax': round((tax_debit_sums + tax_credit_sums) * (tax.amount / 100), 2)
                })
    ...
    @api.model
    def get_filter_values(self, start_date, end_date, comparison_number, comparison_type, options, report_type):
        """
        ...
        if report_type is not None and 'account' in report_type:
            ...
            for account in account_ids:
                tax_ids = self.env['account.move.line'].search([('account_id', '=', account.id)]).mapped('tax_ids')
                if tax_ids:
                    for tax in tax_ids:
                        ...
                        if comparison_number:
                            if comparison_type == 'year':
                                ...
                                for i in range(1, eval(comparison_number) + 1):
                                    ...
                                    tax_id = self.env['account.move.line'].search([('tax_ids', '=', tax.id),
                                                                                   ('date', '>=', com_start_date),
                                                                                   ('date', '<=', com_end_date),
                                ...
                                for i in range(1, eval(comparison_number) + 1):
                                    ...
                                    dynamic_total_tax_sum[
                                        ...
                                        dynamic_total_net_sum[
                                            f"dynamic_total_net_sum{i}"] * (
                                                tax.amount / 100)
                                    dynamic_date_num[f"dynamic_date_num{i}"] = self.get_month_name(
                                        com_start_date) + ' ' + str(com_start_date.year)
            ...
            end_date = end_date_first
            for tax in tax_ids:
                account_ids = self.env['account.move.line'].search([('tax_ids', '=', tax.id)]).mapped('account_id')
                for account in account_ids:
                    dynamic_total_tax_sum = {}
                    dynamic_total_net_sum = {}
                    if comparison_number:
                        if comparison_type == 'year':
                            ...
                            for i in range(1, eval(comparison_number) + 1):
                                ...
                                tax_debit_sums = sum(record['debit'] for record in tax_id)
                                tax_credit_sums = sum(record['credit'] for record in tax_id)
                                dynamic_total_net_sum[f"dynamic_total_net_sum{i}"] = tax_debit_sums + tax_credit_sums
                                dynamic_total_tax_sum[
                                    f"dynamic_total_tax_sum{i}"] = \
                                    dynamic_total_net_sum[
                                        f"dynamic_total_net_sum{i}"] * (
        ...
        return {'dynamic_date_num': dynamic_date_num, 'sale': sale, 'purchase': purchase}
            ...
            if data['report_type']:
                if list(data['report_type'].keys())[0] == 'account':
                    ...
                    if prev_tax != sale['name']:
                        prev_tax = sale['name']
                        sheet.write(row, col, sale['name'] + '(' + str(sale['amount']) + '%)', txt_name)
                        sheet.write(row, col + 1, '', txt_name)
                        sheet.write(row, col + 2, '', txt_name)
                row += 1
                    ...
                    if sale['dynamic net']:
                        ...
                        for num in periods:
                            if sale['dynamic net']['dynamic_total_net_sum' + str(num)]:
                                sheet.write(row, col + j, sale['dynamic net']['dynamic_total_net_sum' + str(num)],
                                            txt_name)
                            if sale['dynamic tax']['dynamic_total_tax_sum' + str(num)]:
                                sheet.write(row, col, sale['dynamic tax']['dynamic_total_tax_sum' + str(num)], txt_name)
                            j += 1
                j = 0
            ...
            if data['report_type']:
                if list(data['report_type'].keys())[0] == 'account':
                    ...
                    if prev_tax != purchase['name']:
                        ...
                        sheet.write(row, col, purchase['name'] + '(' + str(purchase['amount']) + '%)', txt_name)
                        sheet.write(row, col + 1, '', txt_name)
                        sheet.write(row, col + 2, '', txt_name)
                row += 1
                if data['apply_comparison']:
                    if purchase['dynamic net']:
                        periods = data['comparison_number_range']
                        for num in periods:
                            if purchase['dynamic net']['dynamic_total_net_sum' + str(num)]:
                                sheet.write(row, col + j, purchase['dynamic net']['dynamic_total_net_sum' + str(num)],
                                            txt_name)
                            if purchase['dynamic tax']['dynamic_total_tax_sum' + str(num)]:
                                sheet.write(row, col, purchase['dynamic tax']['dynamic_total_tax_sum' + str(num)],
                                            txt_name)
                            j += 1
                j = 0
